...
def get_antinode_locations(grid, freqs, map_of_freqs):
    antinodes = []
    for freq in freqs:
        list_of_locations = map_of_freqs[freq]
        for i in range(len(list_of_locations)):
            antinodes.append(list_of_locations[i])
            for j in range(i+1, len(list_of_locations)):
                ...
                while 0 <= antinode_x < len(grid) and 0 <= antinode_y < len(grid[0]):
...
if __name__ == "__main__":
    ...
    antinodes = get_antinode_locations(grid, freqs, map_of_freqs)
